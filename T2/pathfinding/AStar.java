...
//the horizontal or vertical cost of a generic diagonal movement from one node to another
    private int sidewaysMoveCost = 10; 
...
//for the nodes encoded within this path blocks them from the map.
    ...
    public Path findPath(Waypoint initialWaypoint, Waypoint finalWaypoint){
    	Node start = new Node((int)initialWaypoint.getY(), (int)initialWaypoint.getX());
    	Node destination = new Node((int)finalWaypoint.getY(), (int)finalWaypoint.getX());
        ...
        while (listOpen.size() != 0) {
            Node present = listOpen.poll(); //poll returns and removes the element at the front of the open list
            listClosed.add(present);
            if (present.equals(destination)) {
                return shortenPath(listToPath(getPath(present)));
            } else {
                addAdjacentNodes(present);
            }
        }
        return new Path();
    }
    ...
    private List<Node> getPath(Node currentNode){
        ...
        Node parent;
        while ((parent = currentNode.getParentNode()) != null) {
            path.add(0, parent);
            currentNode = parent;
        }
        return path;
    ...
    private void addAdjacentLowerRow(Node currentNode){
        ...
        if (lowerRow < map.getMap().length) {
            if (col - 1 >= 0) {
                checkNode(currentNode, col - 1, lowerRow, this.diagonalMoveCost); // Comment this line if diagonal movements are not allowed
            }
            if (col + 1 < map.getMap()[0].length) {
                checkNode(currentNode, col + 1, lowerRow, this.diagonalMoveCost); // Comment this line if diagonal movements are not allowed
            }
            checkNode(currentNode, col, lowerRow, this.sidewaysMoveCost);
        }
    }
    ...
    private void addAdjacentMiddleRow(Node currentNode){
        ...
        int middleRow = row;
        if (col - 1 >= 0) {
            checkNode(currentNode, col - 1, middleRow, this.sidewaysMoveCost);
        }
        if (col + 1 < map.getMap()[0].length) {
            checkNode(currentNode, col + 1, middleRow, this.sidewaysMoveCost);
        }
    ...
    private void addAdjacentUpperRow(Node currentNode){
        ...
        if (upperRow >= 0) {
            if (col - 1 >= 0) {
                checkNode(currentNode, col - 1, upperRow, this.diagonalMoveCost); // Comment this if diagonal movements are not allowed
            }
            if (col + 1 < map.getMap()[0].length) {
                checkNode(currentNode, col + 1, upperRow, this.diagonalMoveCost); // Comment this if diagonal movements are not allowed
            }
            checkNode(currentNode, col, upperRow, this.sidewaysMoveCost);
        }
    }
   
    private void checkNode(Node currentNode, int col, int row, int cost){
        Node adjacentNode = map.getMap()[row][col];
        if (!adjacentNode.isNodeOffline() && !this.listClosed.contains(adjacentNode)) {
            if (!this.listOpen.contains(adjacentNode)) {
                adjacentNode.setState(currentNode, cost);
                this.listOpen.add(adjacentNode);
            } else {
                boolean changed = adjacentNode.isThereAShorterRoute(currentNode, cost);
                if (changed) {
                    // Remove and Add the changed node, so that the PriorityQueue can sort again its
                    // contents with the modified "finalCost" value of the modified node
                    this.listOpen.remove(adjacentNode);
                    this.listOpen.add(adjacentNode);
                }
            }
        }
